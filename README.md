> 🔒 **Dokumen ini FINAL — struktur folder & pembagian modul tidak berubah sampai akhir project.** Perubahan hanya boleh dicatat di bagian "Riwayat perubahan" pada [`JOBDESK.md`](./JOBDESK.md), tidak dengan mengedit isi struktur di bawah secara diam-diam.
...
## Struktur proyek (final — locked)

```
SIGMA-project/
├── backend/                      # Reyka — Modul 2, 3, Core Infra
│   ├── app/
...
│   │   ├── services/
│   │   │   ├── scoring/           # algoritma skor rapor A-E
│   │   │   └── rag/               # Modul 3 - RAG chatbot
│   │   ├── workers/                # Celery tasks (health check, scanner trigger)
│   │   └── main.py
...
│   └── Dockerfile
│
├── scanner/                       # Fathir — Modul 1
│   ├── dorking/                   # SerpAPI crawler
│   ├── portscan/                  # Nmap/Nuclei wrapper
│   ├── fingerprint/                # CMS/plugin/SSL check
│   ├── recon/                      # Shodan + VirusTotal
│   ├── screenshots/                # Playwright output
│   └── requirements.txt
│
├── frontend/                      # Ino — Dashboard & Portal Publik
│   ├── src/
│   │   ├── app/ (atau pages/)
│   │   ├── components/
│   │   ├── features/
...
│   └── Dockerfile
│
├── docs/                           # dokumen non-kode
│   ├── JOBDESK.md
│   ├── architecture.md
│   └── sop/                        # dokumen SOP buat RAG Modul 3
│
├── docker-compose.yml
├── .env.example
├── .gitignore
...
**Prinsip pembagian:** `backend/` dan `scanner/` sengaja dipisah — scanner (Modul 1) berjalan sebagai worker/tool yang dipanggil lewat task queue, bukan API server, jadi Reyka bisa konsumsi hasilnya tanpa masuk ke kode Fathir langsung. Ini ngurangin resiko konflik git antar anggota.

Setiap anggota kerja di folder tanggung jawabnya masing-masing (`backend/` = Reyka, `scanner/` = Fathir, `frontend/` = Ino). Folder baru di luar struktur ini **tidak ditambahkan** tanpa dicatat di riwayat perubahan JOBDESK.md.
